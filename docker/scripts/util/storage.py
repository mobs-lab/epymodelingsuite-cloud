...
def load_bytes(path: str, decompress: bool | None = None) -> bytes:
    ...
    decompress : bool | None, optional
        Whether to gzip-decompress the data after loading.
        If None (default), auto-detects from filename (.gz extension)

    Returns
    -------
    bytes
        File contents as bytes (decompressed if decompress=True)
    ...
    # Auto-detect decompression from filename if not explicitly set
...
def save_bytes(path: str, data: bytes, compress: bool | None = None, content_type: str | None = None) -> None:
    ...
    # Auto-detect compression from filename if not explicitly set
    if compress is None:
