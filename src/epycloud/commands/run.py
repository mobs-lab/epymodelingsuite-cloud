...
def _run_workflow_cloud(
    ...
    confirmation_info = _build_base_confirmation_info(ctx, "workflow", exp_id, generated_run_id)
    ...
    if not _prompt_user_confirmation(auto_confirm, confirmation_info, mode="cloud"):
        return 0
...
def _run_workflow_local(
    ...
    # Build confirmation info
    confirmation_info = _build_base_confirmation_info(ctx, "workflow", exp_id, run_id)
    confirmation_info.update({
        "storage_path": storage_path,
        "skip_output": skip_output,
        "output_config": output_config,
    ...
    # Show confirmation and prompt
    if not _prompt_user_confirmation(auto_confirm, confirmation_info, mode="local"):
        return 0
...
def _run_job_cloud(
    ...
    # Build confirmation info
    confirmation_info = _build_base_confirmation_info(
    ...
    _add_stage_specific_info(confirmation_info, stage, task_index, num_tasks, output_config)
...
def _build_base_confirmation_info(
    ctx: dict[str, Any],
    command_type: str,
    exp_id: str,
    run_id: str,
...
def _prompt_user_confirmation(auto_confirm: bool, confirmation_info: dict[str, Any], mode: str) -> bool:
