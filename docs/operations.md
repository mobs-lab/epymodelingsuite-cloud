...
# uv run epycloud terraform init && uv run epycloud terraform apply
...
uv run epycloud run workflow --exp-id experiment-01

# 2. Monitor workflow
uv run epycloud workflow list
uv run epycloud workflow list --exp-id experiment-01
...
uv run epycloud run local builder --exp-id test-sim
# After builder completes, get RUN_ID from output and run runner
uv run epycloud run local runner --exp-id test-sim --run-id <run_id> --task-index 0
# Generate outputs
uv run epycloud run local output --exp-id test-sim --run-id <run_id> --num-tasks 1
```
...
- `uv run epycloud build cloud` - Cloud Build, pushed to Artifact Registry.
- `uv run epycloud build local` - Local build, pushed to Artifact Registry.
- `uv run epycloud build dev` - Development build for locally testing pipeline. Doesn't get pushed to cloud.
...
# Basic run
uv run epycloud run workflow --exp-id my-experiment
...
uv run epycloud workflow list --exp-id my-experiment

# View workflow details
uv run epycloud workflow describe <execution-id>

# View logs
uv run epycloud logs --exp-id my-experiment
uv run epycloud logs --exp-id my-experiment --stage B --tail 500
```
...
for i in {0..9}; do
  uv run epycloud run local runner --exp-id test-sim --run-id <run_id> --task-index $i &
done
wait
...
uv run epycloud run local output --exp-id test-sim --run-id <run_id> --num-tasks $NUM_TASKS
...
```bash
source .env
uv run epycloud terraform init
```
...
```bash
source .env
uv run epycloud terraform plan
```
...
```bash
source .env
uv run epycloud terraform apply
```
...
```bash
source .env
uv run epycloud terraform destroy
```
...
uv run epycloud run workflow --exp-id updated-experiment
```
...
uv run epycloud build cloud
uv run epycloud terraform apply  # Update workflow to reference new tag
```